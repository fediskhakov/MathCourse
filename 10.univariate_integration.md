...
1. Online Test 5 on **Monday May 20**
...
3. Tutorials replaced by consultations in week 12

    - [Same schedule and locations](https://math.iskh.me/index.html#tutorial-times)
    - Questions and answers
    - You can come to multiple sessions through the week

4. My consultation hours
    
    - **Wednesday May 22, 9:30-11:00**
    - [Office 1021, HW Arndt Building](https://math.iskh.me/index.html#course-convener-and-lecturer)
    - Come with specific questions

5. Next lecture: revision and consultation
...
Let $I=(a, b)=\{x \in \mathbb{R}: a<x<b\}$
wherea $\in \mathbb{R}, b \in \mathbb{R}$, and $a<b$

Let $f: I \to \mathbb{R}$ be some univariate real-valued function. 
Suppose that there exists some function $F: I \to \mathbb{R}$ such that
...
$$
G'(t) = \frac{d}{dt} F(\omega(t)) = F'(\omega(t)) \omega'(t) = f(\omega(t))\omega'(t)
$$

By definition of anti-derivative it follows that 

$$
G(t)+ C = \int G'(t) dt = \int f(\omega(t))\omega'(t) dt
$$

On the other hand, when $x=\omega(t)$, $G(t)=F(x)$, and thus

$$
\int f(x) dx = F(x)+C = G(t)+C = \int f(\omega(t))\omega'(t) dt
$$
...
2. You can see a way to express $f(x)$ as $f(x)=g(\omega(t)) \omega^{\prime}(t)$ for which you know the functions $g(x)$ and $\omega(t)$

3. You do know how to calculate the indefinite integral of $g(x)$ with respect to $x$
...
Let's make a substitution $t = \ln(x)$, in which case $x=\exp(t)=\omega(t)$ and $\omega'(t)=\exp(t)$.
We continue

$$
=
\int \frac{\ln (\omega(t))}{\omega(t)}\omega'(t) dt = 
\int \frac{\ln (\exp(t))}{\exp(t)}\exp(t) dt = \\
\int t dt = t^2/2 + C = (\ln(x))^2/2 + C
$$
...
$$
\int f(x) dx=\int e^{x^{2}} x dx =
$$

Let use a new variable $t=x^2$, in other words $x=\sqrt{t}=\omega(t)$, $\omega'(t) = \frac{1}{2\sqrt{t}}$.
Continuing

$$
=
\int e^{t} \sqrt{t} \frac{1}{2\sqrt{t}} dt =
\frac{1}{2} \int e^{t} dt =
...
$$
\frac{d}{dx}\left(\frac{1}{2} e^{x^{2}}\right)=\frac{1}{2}(2 x) e^{x^{2}}=x e^{x^{2}}=e^{x^{2}} x
$$
```
...
Note that $\int u^{\prime} v dx$ and $\int v^{\prime} u dx$ both implicitly incorporate arbitrary constants
...
$$
\lim_{n \to \infty} S(T_n) = \int_{a}^{b} f(x) dx \quad \text{for all} \quad 
T_n: \lim_{n \to \infty} r(T_n) = 0
$$
...
```{image} _static/img/notes/14may/1.png
   :width: 75%
```
