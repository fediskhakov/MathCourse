...
We want to extend our discussion of differential calculus from univariate real-valued functions, $f(x_1)$, to multivariate real-valued functions, $f(x_1,x_2,\dots,x_n)$.
...
Illustrations of domain and range of $f$.
```
...
The formal definition of the partial derivative of the single-real-valued function $f$ with respect to the variable $x_{k}$ is

$$
\begin{aligned}
\frac{\partial f\left(\mathbf{x}\right)}{\partial x_{k}} = \lim _{h \rightarrow 0}\left\{\frac{f\left(x_{1}, \dots, x_{k} + h, x_{k+1}, \dots, x_n\right)}{h}\right\} .
\end{aligned}
$$
```

The partial derivative is approximately equal to the per-unit change in $f$ caused by an increase in $x_i$, while holding everything else constant.
...
As partial derivatives of multivariate functions is similar to derivatives of univariate function, the same differentiation rules apply.
...
Note that when calculating the partial derivative with respect $x_k$ we treat the remaining independent variables as constants.
...
In economic analysis the partial derivative often has a straight forward interpretation
...
We will provide some illustrations of an application of the concept of a partial derivative by using a variety of different two-good utility functions. These utility functions take the form $f: \mathbb{R}_{+}^{2} \rightarrow \mathbb{R}$.

$$
U=U\left(Q_{1}, Q_{2}\right)
$$

where $U$ is the level of utility, $Q_{1}$ is the amount of good one that is consumed and $Q_{2}$ is the amount of good two that is consumed.

We will consider a linear utility function, a Cobb-Douglas utility function and a CES utility function.


```{admonition} Linear utility function
:class: tip
Suppose that we have a linear utility function:
...
```{admonition} Marginal products of a Cobb-Douglas production function
:class: tip

Suppose that we have a Cobb-Douglas production function where labor (L) and capital (K) are the inputs:
...
```{admonition} Elasticities of the Cobb-Douglas production function
:class: tip
...
- If these $n$ partial derivative functions are continuous, we say that $f$ is continously differentiable at $\mathbf{x}$.
- If $f$ is twice-differentiable at every point $x \in X$, then $f$ is said to be twice-differentiable on $X$. This is denoted by $f \in C^{1}$ on $X$.
...
The total derivative, $df$, can be thought of as a linear approximation of the change in $f$ due to the change $dx$.

$$
df \approx f(\mathbf{x} + dx) - f(\mathbf{x}).
$$
...
The growth rate in the production is constant, and is given by dot product of the elasticities and growth rates with respect to each of the production inputs.
