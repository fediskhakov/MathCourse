...
1. Results of Online test 2

```{figure} _static/img/notes/EMET7001quiz2.png
:width: 80%

Distribution of results for Online test 2
```

2. Teaching break: next two weeks **April 1st to 14th**
  
  - No teaching
  - No tutorials
  - Next lecture is on **April 16th**
  - Next online test is on **April 22nd**, after the next lecture
...
{cite:ps}`spivak2006` (p. 90) provides the following informal definition of a limit: “The function $f$ approaches the limit $l$ near [the point $x =$] $a$, if we can make $f(x)$ as close as we like to $l$ by requiring that $x$ be sufficiently close to, but unequal to, $a$.”

```{admonition} Definition
:class: caution

The function $f: \mathbb{R} \rightarrow \mathbb{R}$ approaches the limit $l$ near [the point $x = a$] if for every $\epsilon > 0$, there is some $\delta > 0$ such that, for all $x$ such as $0 < |x − a| < \delta$, $|f(x) − l | < \epsilon$.
```
...
We will also use $d(x,a)$ to denote distance, i.e. $d(x,a) = |x-a|$.
...
where $\binom{n}{k} = C_{n,k} = \frac{n!}{k! (n − k)!}$ is known as a binomial coefficient (read "$n$ choose $k$" because this is the number of ways to choose a subset of $k$ elements from a set of $n$ elements).
...
$$
f^{(k)}(x)=\frac{d^{k} f}{\underbrace{dx \cdots dx}_{k}}=
\frac{d}{d x}\left(\frac{d^{k-1} f}{\underbrace{dx \cdots dx}_{k-1}}\right)
$$
...
```{admonition} Definition (in words)
:class: caution
...
$$
Q &= a/P = a P^{-1} \quad \Leftrightarrow \quad \ln Q = \ln a - \ln P \\
...
```{image} _static/img/notes/26mar/1.png
   :width: 75%
```
```{image} _static/img/notes/26mar/2.png
   :width: 75%
```
```{image} _static/img/notes/26mar/3.png
   :width: 75%
```
```{image} _static/img/notes/26mar/4.png
   :width: 75%
```
```{image} _static/img/notes/26mar/5.png
   :width: 75%
```
...
- Great video on Taylor series [YouTube Morphocular](https://youtu.be/0HaBNdmUWXY?si=Fcps8tuAvEJMOyOR)
```