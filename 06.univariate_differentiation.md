...
- {cite:ps}`6-bradley2008`: Chapter 6 (pp. 257–357).
- {cite:ps}`6-haeussler1987`: Chapters 10, 11, 12, and 13 (pp. 375–532).
- {cite:ps}`6-shannon1995`: Chapter 8 (pp. 356–407).
- {cite:ps}`6-sydsaeter2016`: Chapters 6, 7, and 8.

### More advanced references
- {cite:ps}`6-ayres2013`
- {cite:ps}`6-chiang2005`: Chapters 6–10 (pp. 124–290).
- {cite:ps}`6-kline1967`: Chapters 2, 4, 5, 6, 7, 12, 13, and 20.
- {cite:ps}`6-silverman1969`: Chapters 4, 5, and 6.
- {cite:ps}`6-simon1994`:
- {cite:ps}`6-spivak2006`: Chapters 3–12 (pp. 39–249).
...
{cite:ps}`6-spivak2006` (p. 90) provides the following informal definition of a limit: “The function $f$ approaches the limit $l$ near [the point $x =$] $a$,
if we can make $f(x)$ as close as we like to $l$ by requiring that $x$ be sufficiently close to, but unequal to, $a$.”

{cite:ps}`6-spivak2006` (p. 96) provides the following formal definition of a limit: “The function $f$ **approaches the limit $l$ near** [the point $x =$] $a$ means: for every $\epsilon > 0$, there is some $\delta > 0$ such that, for all $x$, if 
...
Limits can sometimes exist even when the function being considered is not so well behaved. One such example is provided by {cite:ps}`6-spivak2006` (pp. 91–92). It involves the use of a trigonometric function.
...
We want to show that this implies that $f(x)$ is continuous at the point $a \in X$.  The following proof of this proposition is drawn from {cite:ps}`6-ayres2013` (Chapter 8, Solved Problem 2).
...
- Let $f(x)=x^{n}$, where $n \in \mathbb{N}$ and $n>1$. Then $f^{\prime}(x)=n x^{n-1}$. Clearly $n x^{n-1}$ is defined for all $x \in \mathbb{R}$. Note that $n x^{n-1}=0$ if and only if $x=0$. Thus the only critical point for this function occurs when $x=0$.

Example 2
- Let $f(x)=\ln (x)$, where $x \in(0, \infty)$. Then $f^{\prime}(x)=\frac{1}{x}$. Note that $\frac{1}{x}$ is not defined when $x=0$. However, the point $x=0$ is NOT a critical value of the function $\ln (x)$. The reason for this is that the point $x=0$ does not belong the domain of this function, which is the open interval $(0, \infty)$. Furthermore, we know that $\frac{1}{x} \neq 0$ for any choice of $x$. Thus we can conclude that there are no critical values for the function $\ln (x)$.
...
This is an extension of Worked Example 4.9 in {cite:ps}`6-bradley2008` (pp. 162-163).
