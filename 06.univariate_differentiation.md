...
To link the limit of the function to the limit of a sequence, we can use the folloing *equivalent* definition
```{admonition} Definition
:class: caution

The function $f: \mathbb{R} \rightarrow \mathbb{R}$ approaches the limit $l$ near [the point $x =$] $a$ if for every sequence $\{x_n\}$ such that $x_n \to a$, we have $f(x_n) \to l$.
```
...
Examples of indeterminate forms of limits
