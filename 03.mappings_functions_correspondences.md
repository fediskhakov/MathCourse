...
Monotonic transformation of a give function is its composition with a a strictly increasing or strictly decreasing function.
```
...
A graph of a mapping $f \colon X \to Y$ is a set $\Gamma \subset X \times Y$ of points $x \in X$ and $y \in Y$ where it holds $f(x)=y$.
```
